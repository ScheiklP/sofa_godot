...
const ARGUMENT_NAME = "add_collision_model_func"

var _callable: AddCollisionModel
var _registry = PropertyWrapperRegistry.new(self)

func _init():
	set_visible(false)
	set_name(ARGUMENT_NAME)
	_callable = AddCollisionModel.new(self, _registry, true)
	#TODO: supply list specifying which arguments to disable when used as argument
