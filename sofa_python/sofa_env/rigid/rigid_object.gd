...
##
## @desc: Encapsulates sofa_env.sofa_templates.rigid.RigidObject
##
#class_name SofaEnvRigidObject
...
const OBJECT_NAME = "rigid_object"
...
func _init():
	...
	_statement = PyCallableStatement.new(RigidObject.new(self, _registry))
	_statement.add_plugin_list(RigidObject.MODULE, RigidObject.MODULE.plugin_list)
	_py_props.register()
...
class RigidObject:
	...
	const CLASS_NAME = "RigidObject"

	const cat_rigid = "Rigid Object"
	...
	func _setup_properties():
		...
		_registry.make_string(PyContext.PY_NONE, "collision_mesh_path")\
			...
			.category(cat_rigid)\
			.callback(self, "_on_update_mesh")
		## Whether to add a "FixedConstraint" to hold the object in place with its original position.
		_registry.make_bool(false, "fixed_position").category(cat_rigid)
		## Whether to add a "FixedRotationConstraint" to hold the object in place with its original orientation
		_registry.make_bool(false, "fixed_orientation").category(cat_rigid)
	...
	func _setup_callable():
		...
		## Whether to add a "FixedConstraint" to hold the object in place with its original position.
		args.add_property("fixed_position")
		## Whether to add a "FixedRotationConstraint" to hold the object in place with its original orientation
		args.add_property("fixed_orientation")
		## Total mass of the rigid object.
